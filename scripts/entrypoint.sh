...
has_custom_endpoint_config() {
  ...
  if [[ -n "${OPENAI_API_KEY:-}${CUSTOM_API_KEY:-}" ]]; then
    return 0
  fi
...
if ! has_valid_provider_config; then
  if [[ -n "${CUSTOM_BASE_URL:-}${OPENAI_BASE_URL:-}" ]]; then
    echo "[bootstrap] ERROR: Custom/OpenAI-compatible endpoints require CUSTOM_BASE_URL or OPENAI_BASE_URL plus OPENAI_API_KEY or CUSTOM_API_KEY." >&2
  else
...
for key in \
  OPENROUTER_API_KEY CUSTOM_BASE_URL CUSTOM_API_KEY OPENAI_API_KEY OPENAI_BASE_URL ANTHROPIC_API_KEY MINIMAX_API_KEY MINIMAX_BASE_URL MINIMAX_CN_API_KEY MINIMAX_CN_BASE_URL HERMES_MODEL MODEL_NAME HERMES_INFERENCE_PROVIDER HERMES_INFERENCE_MODEL HERMES_TUI_PROVIDER HERMES_PORTAL_BASE_URL NOUS_INFERENCE_BASE_URL HERMES_NOUS_MIN_KEY_TTL_SECONDS HERMES_DUMP_REQUESTS \
